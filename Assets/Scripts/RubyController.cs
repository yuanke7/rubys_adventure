...
public class RubyController : MonoBehaviour
{
    ...
    {
        // 获取摇杆
        var joyStickBg = GameObject.FindGameObjectWithTag("JoyStick");
        var joyStickBgComponent = joyStickBg.GetComponent<Joystick>();
        Vector2 contentOffset = joyStickBgComponent.content.anchoredPosition;
        _horizontal = (float)Math.Round(contentOffset.x, 1) ;
        _vertical =(float)Math.Round(contentOffset.y, 1) ;
        // if (_horizontal != 0)
        // {
        //     _horizontal = NormalizeToOne(_horizontal);
        // }
        // if (_vertical != 0)
        // {
        //     _vertical = NormalizeToOne(_vertical);
        // }
        // Debug.Log($"2222{_horizontal} {_vertical}");

        // Debug.Log(contentOffset);
        // 获取输入的方向x,y
        ...
        if (!Mathf.Approximately(move.x, 0.0f) || !Mathf.Approximately(move.y, 0.0f)) // 如移动的值x or y不为0 设置新的值
        {
            ...
            lookDirection.Normalize();  // 使长度为1
            // Debug.Log($"Normalized _lookDirection {_lookDirection.x} {_lookDirection.y}");
        }
        // 为0则应用之前的值
        _animator.SetFloat(Property, lookDirection.x);
        _animator.SetFloat(Property1, lookDirection.y);
        _animator.SetFloat(Speed, move.magnitude);

        
        if (_horizontal != 0 && _vertical != 0)
        {
            Debug.Log($"MOVE x:{_horizontal} y:{_vertical}");
        }
        // 获取对象位置 向量 进行移动
        var objPosition = _rigidbody2D.position;
        // objPosition.x -= 0.1f;
        objPosition.x += speed * _horizontal/100 * Time.deltaTime; // Time.deltaTime 为一帧的运行时间  
        objPosition.y += speed * _vertical/100 * Time.deltaTime;
        // 更新对象位置到新的位置
    ...
    private void Launch()
    {
        PlaySound(launchSound, 0.15f);
        // Quaternion.identity 表示“无旋转”
        var projectileObject = Instantiate(projectilePrefab, _rigidbody2D.position + Vector2.up * 0.5f,Quaternion.identity);  // 初始化对象
        // var projectile = projectileObject.GetComponent<Projectile>();
        var projectileRigi = projectileObject.GetComponent<Rigidbody2D>();
        projectileRigi.velocity = new Vector2(lookDirection.x * bulletSpeed, lookDirection.y * bulletSpeed);   // 不使用物理系统发射
        // projectile.Launch(director: lookDirection, force: 500);
    ...
    public int NormalizeToOne(float num)
    {
        int ret;
        if (num > 0)
        {
